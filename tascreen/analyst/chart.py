...
def _merge(a: str, b: str) -> str:
    """Two labels at one height: one number with both names when they share a price
    (review round 3: the same number twice in one label), else one above the other."""
    ma, mb = LABEL.match(a), LABEL.match(b)
    if ma and mb and ma.group(2) == mb.group(2) and "\n" not in a + b:
        return f"{ma.group(1)} ו{mb.group(1)} \u2066{ma.group(2)}\u2069"
    return f"{a}\n{b}"
...
class _Tags:
    ...
    places and hid axis values); a label pushed off its level gets a thin leader. One size
    for all (round 3: merged labels shrank to 9.5 px): a label too wide takes a second line."""

    SIZE, LINE, CHAR = 13.5, 17, 7.8         # font px, px per line, px per character
    ...
    def add(self, x: float, y: float, text: str, color: str, anchor: str = "start") -> None:
        if anchor != "middle":
            ...
            return
        width = 16 + self.CHAR * self._chars(text)
        ...
        self._draw(left, y, width, [text], color)

    def _wrap(self, text: str, room: float) -> list[str]:
        lines = []
        for line in text.split("\n"):
            pieces = line.split(" · ")
            current = pieces[0]
            for piece in pieces[1:]:
                if 16 + self.CHAR * self._chars(f"{current} · {piece}") <= room:
    ...
    def finish(self) -> list[str]:
        ...
        for y, text, color in sorted(self.pending, key=lambda t: t[0]):
            if tags and y - tags[-1][0] < 10:            # two levels at one height: one label
                tags[-1] = ((tags[-1][0] + y) / 2, _merge(tags[-1][1], text), tags[-1][2])
            else:
                tags.append((y, text, color))
        left = self.plot.xc + 8
        room = self.plot.x1 - left - 2                # never into the profile or the axis
        laid = [(y, self._wrap(text, room), color) for y, text, color in tags]
        half = [(8 + self.LINE * len(lines)) / 2 for _, lines, _ in laid]
        ys: list[float] = []
        for k, (wanted, _, _) in enumerate(laid):
            low_edge, high_edge = self.plot.y0 + half[k] + 1, self.plot.y1 - half[k] - 1
            y = min(max(wanted, low_edge), high_edge)
            if ys:
                y = max(y, ys[-1] + half[k - 1] + half[k] + 4)
            ys.append(y)
        for k in range(len(ys) - 1, -1, -1):          # the bottom overflowed: push back up
            limit = self.plot.y1 - half[k] - 1 if k == len(ys) - 1 else ys[k + 1] - half[k + 1] - half[k] - 4
            ys[k] = max(min(ys[k], limit), self.plot.y0 + half[k] + 1)
        for (wanted, lines, color), y in zip(laid, ys):
            width = min(room, 16 + self.CHAR * max(self._chars(line) for line in lines))
            ...
            self._draw(left, y, width, lines, color)
        self.pending = []
        return self.out

    def _draw(self, left: float, y: float, width: float, lines: list[str], color: str) -> None:
        height = 8 + self.LINE * len(lines)
        texts = "".join(
            f'<text x="{left + width / 2:.1f}" y="{y - height / 2 + 4 + self.LINE * (k + 0.5) + 4.5:.1f}" '
            f'fill="{color}" font-family="{SANS}" font-size="{self.SIZE}" font-weight="600" '
            f'text-anchor="middle">{_esc(chr(0x2067) + line + chr(0x2069))}</text>'   # right to left, whatever it starts with
            for k, line in enumerate(lines))
        self.out.append(f'<g class="ann"><rect x="{left:.1f}" y="{y - height / 2:.1f}" width="{width:.1f}" '
                        f'height="{height:.1f}" rx="6" fill="{ANN["tag"]}" stroke="{color}" stroke-width="1"/>'
                        f'{texts}</g>')
...
def render(bars: pd.DataFrame, analysis: Analysis, drawings: list[str] | None = None, *,
    ...
    if drawings is None:
        ...
        if cited is not None:
            ...
            if not any(c.startswith("vp.") for c in cited):
                items.pop("vp", None)
                gone.append("poc")
            for key, item in list(items.items()):     # nor in a band's label (review round 3)
                if item.get("type") == "zone" and item.get("notes"):
                    items[key] = {**item, "notes": [n for n in item["notes"] if not n.startswith(tuple(gone or ["-"]))]}
    ...
    def pattern_levels(key: str, item: dict[str, Any]) -> dict[str, float]:
        ...
        named = cited is None or f"{key}.invalidation" in cited      # round 3: a cancel the text never named
        if named and _finite(cancel_at) and (not math.isfinite(atr) or abs(float(cancel_at) - close) <= PATTERN_LEVEL_ATR * atr):
    ...
    for item in items.values():
        ...
        elif item["type"] in ("extreme", "level"):
    ...
    sub = (f'<tspan fill="{INK["title"]}">{_fmt(close)}</tspan>'
           ...
           + f'  ·  יומי  ·  {pd.Timestamp(days[last]):%d/%m/%Y}')
    ...
    kinds = {v.get("kind") for v in items.values() if v.get("type") in ("zone", "level")}
    ...
    legend = [(ZONE["support"], "תמיכה") if "support" in kinds else None,
              ...
              *[(MA_COLORS[n], f"ממוצע {n} יום") for n in periods if n in MA_COLORS],
              ...
              (INK["axis"], "קו מגמה") if any(v["type"] == "line" for v in items.values()) else None,
    ...
    for item in items.values():
        ...
        x_a = plot.x(max(first, index.get(item.get("first_day") or "", first))) - plot.step
        y_a, y_b = plot.y(item["high"]), plot.y(item["low"])
        name = item.get("label") or ("תמיכה" if item["kind"] == "support" else "התנגדות")
        if item["high"] - item["low"] < 1e-9:                    # one price: a line
            out.append(f'<line class="ann" x1="{x_a:.1f}" y1="{y_a:.1f}" x2="{right:.1f}" y2="{y_a:.1f}" '
                       f'stroke="{color}" stroke-width="1.3" stroke-opacity="0.8" stroke-dasharray="8 4"/>')
            text = f"{name} {_ltr(_fmt(item['low']))}"
        else:
            ...
            text = f"{name} {_ltr(_fmt(item['low']) + '–' + _fmt(item['high']))}"
        # the pattern's line gets its own label with its price
        notes = " · ".join(word + (f" {_ltr(number)}" if number else "")
                           for word, number in note_labels([n for n in item.get("notes", []) if n != "pat"]))
        tags.add(right, (y_a + y_b) / 2, text + (f"\n{notes}" if notes else ""), color)

    # volume and candles
    vols = win["volume"].astype(float)
    median = float(vols.median()) if len(vols) else 0.0
    vmax = float(min(vols.max(), 3 * median) if median > 0 else vols.max()) or 1.0
    ...
    for i, row in zip(range(first, last + 1), win.itertuples(index=False)):
        ...
        vh = (plot.v1 - plot.v0) * min(1.0, float(row.volume) / vmax if vmax else 0)
    ...
    if "ma" in items:
        for n in items["ma"]["periods"]:
            ...
            if len(pts) > 1:
                width, opacity = (1.6, 0.9) if n == 50 else (1.0, 0.7)
                dash = ' stroke-dasharray="5 4"' if n == 150 else ""
    ...
    for key, item in items.items():
        ...
        if kind == "line":
            ...
            if (item["kind"] == "support") == (item["price2"] > close):     # the price crossed it
                name += " (נחצה)"
            tags.add(right, plot.y(yb), f"{name} {_ltr(_fmt(item['price2']))}", color)
        elif kind == "fib":
            ...
            color = ZONE.get(item.get("kind"), INK["title"])
            price = float(item["price"])
            if keep(price):
                y = plot.y(price)
                out.append(f'<line class="ann" x1="{plot.x(max(first, last - 30)):.1f}" y1="{y:.1f}" x2="{right:.1f}" '
                           f'y2="{y:.1f}" stroke="{color}" stroke-width="1.2" stroke-opacity="0.8" stroke-dasharray="3 3"/>')
                tags.add(right, y, f"{item['label']} {_ltr(_fmt(price))}".strip(), color)
            else:
                edge = plot.y0 if price > close else plot.y1
                tags.add(right, edge, f"{'↑' if price > close else '↓'} {item['label']} {_ltr(_fmt(price))}".strip(), color)
            ...
            if b is not None and b >= first and _finite(item.get("breakout")):
                ...
                if _finite(now):                            # its price today (review round 3)
                    tags.add(right, plot.y(float(now)),
                             f"{'קו הפריצה' if up else 'קו השבירה'} {_ltr(_fmt(float(now)))}", color)
                mark = ANN["bull"] if up else ANN["bear"]
                x = plot.x(b)
                # beside the candle, on the side the price broke to (review round 3: a
                # breakdown marked at the top of the wedge)
                if up:
                    y = plot.y(float(bars["high"].iloc[b])) - 10
                    tri = f"M{x:.1f},{y - 9:.1f} L{x - 6:.1f},{y:.1f} L{x + 6:.1f},{y:.1f} Z"
                else:
                ...
                for offset in (26, 50, 74, 98, 122):
                    ...
                    if all(plot.y(float(bars["low"].iloc[i])) < label_y - 12 or
                        ...
                        break
            confirm = item.get("breakout")
            if item.get("status") == "forming" and _finite(confirm) and keep(confirm):
                y = plot.y(float(confirm))
                out.append(f'<line class="ann" x1="{plot.x(max(first, index.get(item["end"], first))):.1f}" '
                           f'y1="{y:.1f}" x2="{right:.1f}" y2="{y:.1f}" stroke="{color}" stroke-width="1.3" '
                           f'stroke-dasharray="5 4"/>')
                word = "קו השבירה" if item.get("direction") == "bearish" else "קו הפריצה"
                tags.add(right, y, f"{word} {_ltr(_fmt(float(confirm)))}", color)
            names = {"target": ("יעד התבנית", "7 4"), "invalidation": ("ביטול התבנית", "2 4")}
            for which, level in pattern_levels(key, item).items():
                if not keep(level):
                    if which == "target":              # far off: an edge tag with the price
                        up_t = level > close
                        tags.add(right, plot.y0 if up_t else plot.y1,
                                 f"{'↑' if up_t else '↓'} יעד התבנית {_ltr(_fmt(level))}", color)
                    continue
